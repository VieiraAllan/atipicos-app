...
└─ src/
   ...
   ├─ constants/
   │   ├─ formData.ts           # configuração dos campos de cada formulário
   │   └─ areaData.ts           # dados (mock) das áreas internas
   ├─ utils/
   │   ├─ masks.ts              # máscaras de CPF / CEP / data
   │   └─ speech.ts             # voz pt-BR (expo-speech) — RF11/RF13
   ├─ components/               # Logo, BackButton, PrimaryButton, Field, ...
   │   └─ internas/             # TopBar, BottomNav, AppShell, Card, widgets,
   │                            # BackChip, PatientCard
   ├─ services/
   ...
   └─ screens/
       ├─ (auth/cadastro)       # Splash, Inicial, CadastroEscolha, Login,
       │                        # CadastroAdulto, CadastroCrianca,
       │                        # RecuperacaoSenha, Sucesso
       ├─ kids/                 # HomeKids, Emoções, Tarefas, Fono, Psico,
       │                        # Escola, Comunicação (CAA), SOS
       ├─ pais/                 # HomeResponsavel, EditarTarefas, Controle,
       │                        # Relatorios, Localizacao, Noticias, Config
       ├─ terapeuta/            # HomeTerapeuta, Paciente, Config
       └─ internas/ConfigScreen # configurações (reutilizada)
```
...
Login(atipico)     → HomeKids → {Emoções, Tarefas, Fono, Psico, Escola, Comunicação, SOS}
Login(responsavel) → HomeResponsavel → {EditarTarefas, Controle, Relatorios,
                                        Localizacao, Noticias, Config, CadastroCrianca}
Login(terapeuta)   → HomeTerapeuta → {Paciente (relatório de sessão), Config}
```
