  ...
  return (
    ...
    >
      <Card title="Rotina da Criança">
        <Linha emoji="🦷" texto="Escovar os dentes" hora="08:00" done />
        <Linha emoji="🎒" texto="Ir para a escola" hora="09:00" />
        <Linha emoji="📚" texto="Lição de casa" hora="15:00" />
      </Card>
...
function Linha({ emoji, texto, hora, done }: { emoji: string; texto: string; hora: string; done?: boolean }) {
  return (
    <View style={[styles.task, done && styles.taskDone]}>
      <View style={[styles.chk, done && styles.chkOn]}>{done && <Text style={styles.chkMark}>✓</Text>}</View>
      <Text style={styles.taskEmoji}>{emoji}</Text>
      <Text style={styles.taskText}>{texto}</Text>
      <Text style={styles.taskTime}>{hora}</Text>
    </View>
  );
}
