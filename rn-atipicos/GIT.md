...
— que **já existem em React Native** neste projeto (pastas `src/screens/kids/`,
`src/screens/pais/`, `src/screens/terapeuta/`).

```bash
git checkout feature/auth-cadastro
git checkout -b plano-b-completo

# copie/garanta que as pastas internas estão presentes:
#   src/screens/kids/  src/screens/pais/  src/screens/terapeuta/
#   src/screens/internas/  src/components/internas/
#   src/constants/areaData.ts  src/utils/speech.ts
# e que o RootNavigator + LoginScreen já apontam os logins para as homes reais
# (HomeKids / HomeResponsavel / HomeTerapeuta) — neste projeto já apontam.

git add -A
git commit -m "feat: app completo navegável (plano B)

- Área Kids: emoções, rotina, fono, psico, comunicação (CAA com voz), escola, SOS
...
> A pasta `prototipo/` (protótipo HTML navegável) acompanha esta branch como
> referência visual viva para a equipe.
