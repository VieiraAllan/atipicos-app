...
- **Storage** → *Get started* → produção.

## 4. Publicar as regras de segurança

Os arquivos `firestore.rules` e `storage.rules` já estão no projeto.

**Pelo console (rápido):** cole o conteúdo de cada arquivo na aba **Rules** do
Firestore e do Storage e clique em **Publicar**.

**Pela CLI (recomendado):**
```bash
npm install -g firebase-tools
firebase login
firebase init firestore storage   # aponte para firestore.rules e storage.rules
...
criancas/{id}
  ...
  diagnostico: string
  observacoes: string
  fotoUrl: string | null           // preenchido pelo upload (RF18)
  criadoEm: timestamp
```
...
- **RF18** `uploadFotoCrianca` (Storage)
